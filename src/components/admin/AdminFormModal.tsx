      ...
      {open && (
        <>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95, y: 8 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 8 }}
            transition={{ duration: 0.2 }} className="fixed inset-0 z-[61] flex items-center justify-center p-4"
                ...
                {fields.map(f => (
                  <div key={f.key}>
                    ...
                    {f.type === "textarea" ? (
                      ...
                      <input type="number" value={data[f.key] ?? ""} onChange={e => setData(d => ({ ...d, [f.key]: parseInt(e.target.value) || 0 }))}
